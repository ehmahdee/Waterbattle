...
const Singleplayer = () => {
  ...
  */
        ...
        function generate(ship) {
            ...
            do {
                ...
                if (!isAtLeftEdge && !isAtRightEdge) {
                    isTaken = current.some((index) => computerSquares[randomStart + index].classList.contains('taken'));
                }
            } while (isTaken);
        ...
        function generate(ship) {
            ...
            const isTaken = current.some(index => computerSquares[randomStart].classList.contains('taken'))
            const isAtRightEdge = current.some(index => (randomStart + index) % width === width - 1)
            const isAtLeftEdge = current.some(index => (randomStart + index) % width === 0)
